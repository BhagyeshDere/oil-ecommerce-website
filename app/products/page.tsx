...
import { ShoppingCart, SlidersHorizontal, Star, Check } from "lucide-react";
...
export default function ProductsPage() {
  ...
  const handleAddToCart = (oil: any) => {
    // 3. Add to actual cart state
    addToCart({
      ...oil,
      image: oil.images[0],
      quantity: 1
    ...
    setAddedItems((prev) => [...prev, oil.id]);
    setTimeout(() => {
      setAddedItems((prev) => prev.filter((id) => id !== oil.id));
    }, 2000);
  };

  return (
    <section className="relative bg-[#fcfcf9] pt-28 pb-20 px-4 md:px-8 overflow-hidden min-h-screen">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {oils.map((oil, i) => {
            const isAdded = addedItems.includes(oil.id);
            ...
            return (
              ...
              >
                {/* Product Detail Link */}
                <Link href={`/products/${oil.id}`} className="absolute inset-0 z-10" />
                ...
                <div className="px-4 py-5 space-y-3 relative z-20">
                  <div className="text-center">
                    ...
                    <p className="text-gray-400 text-[11px] uppercase">5 Litres</p>
                  </div>
                  ...
                  <button
                    ...
                    className={`w-full mt-2 h-11 rounded-xl flex items-center justify-center gap-2 text-xs font-black uppercase tracking-widest transition-all ${
                      isAdded 
                      ? "bg-[#0f7b65] text-white shadow-inner" 
                      : "bg-[#0b3d33] text-white hover:bg-[#0f7b65] hover:scale-[1.02]"
                    }`}
                  >
                ...
                {/* HOVER BORDER */}
                <div className="absolute inset-0 rounded-[2rem] border border-transparent group-hover:border-[#c8a24c]/30 transition-all pointer-events-none" />
              </motion.div>
            );
          })}
        </div>

        {/* CART FLOATING ACTION (Procedure Signal) */}
        <AnimatePresence>
          {addedItems.length > 0 && (
            <motion.div
              ...
              className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 bg-[#0b3d33] text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-6 border border-[#c8a24c]/30"
            >
              ...
              <Link 
                href="/cart" 
                className="bg-[#c8a24c] text-[#0b3d33] px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest hover:bg-white transition-colors"
              >
                Checkout Now
              </Link>
            </motion.div>
